...
from vision_getter_v8.msg import VisionDetectedObjects
...
def image_callback(img_msg):
    ...
    # Show the original image and/or image with boxes
    #show_image(cv_image)
    show_image(cv_img)
    

    # Send boxes info in publisher node
    for i in range(len(boxes)):
        if i in indexes:
            msg.label = str(classes[class_ids[i]])
            msg.confidence = confidences[i]
            msg.distance = 50.0
            print("message to send / Label : %s, Confidence : %f, Distance : %f " % (msg.label, msg.confidence, msg.distance))
            rospy.loginfo(msg)
            pub.publish(msg)

def vision_device_client():
    node = rospy.init_node('vision_getter', anonymous=True)
